#!/bin/bash
# HY-Motion entrypoint with crash recovery — same shape as hybrik-x.
# If server.py crashes, expose the traceback on port 8000 so the gateway
# health-check sees the error instead of a connection refused.

cd /app/HY-Motion-1.0
...
" 2>/dev/null || echo '{"status":"crashed","detail":"unknown error"}')
    ...
    echo "[hy-motion] server.py crashed (exit $EXIT_CODE) — serving error on port 8000"
...
print('[hy-motion] Error server listening on :8000', flush=True)
