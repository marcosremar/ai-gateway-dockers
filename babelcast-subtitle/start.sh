...
# Install llama-cpp-python at runtime — MUST use pre-built wheel only (no compilation).
# --only-binary prevents pip from falling back to source builds which OOM the container.
if ! python3 -c "import llama_cpp" 2>/dev/null; then
    echo "[babelcast-subtitle] Installing llama-cpp-python (pre-built cu128 wheel)..."
    pip install --no-cache-dir --prefer-binary --only-binary llama-cpp-python \
        "llama-cpp-python[server]>=0.3.19" \
        --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu128 2>&1 | tail -5
